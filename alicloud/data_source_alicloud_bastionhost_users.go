...
func dataSourceAlicloudBastionhostUserPublicKeys() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceAlicloudBastionhostUserPublicKeysRead,
		Schema: map[string]*schema.Schema{
			"instance_id": {
				Type:     schema.TypeString,
				Required: true,
...
func dataSourceAlicloudBastionhostUserPublicKeysRead(d *schema.ResourceData, meta interface{}) error {
	client := meta.(*connectivity.AliyunClient)
	...
	request["PageSize"] = PageSizeLarge
	request["PageNumber"] = 1
	var keyNameRegex *regexp.Regexp
	if v, ok := d.GetOk("key_name_regex"); ok {
		r, err := regexp.Compile(v.(string))
		if err != nil {
			return WrapError(err)
		}
	...
	for {
		...
		addDebug(action, response, request)
		if err != nil {
			return WrapErrorf(err, DataDefaultErrorMsg, "alicloud_bastionhost_user_public_keys", action, AlibabaCloudSdkGoERROR)
		}
		resp, err := jsonpath.Get("$.PublicKeys", response)
		if err != nil {
			return WrapErrorf(err, FailedGetAttributeMsg, action, "$.PublicKeys", response)
		}
		result, _ := resp.([]interface{})
		for _, v := range result {
			item := v.(map[string]interface{})
			if keyNameRegex != nil && !keyNameRegex.MatchString(fmt.Sprint(item["PublicKeyName"])) {
				continue
			}
			objects = append(objects, item)
		}
	...
	for _, object := range objects {
		mapping := map[string]interface{}{
			"public_key_id":   fmt.Sprint(object["PublicKeyId"]),
			"public_key_name": object["PublicKeyName"],
			"finger_print":    object["FingerPrint"],
			"comment":         object["Comment"],
		}
		ids = append(ids, fmt.Sprint(mapping["public_key_id"]))
		s = append(s, mapping)
	}

	d.SetId(dataResourceIdHash(ids))
	if err := d.Set("key_ids", ids); err != nil {
		return WrapError(err)
	}

	if err := d.Set("public_keys", s); err != nil {
		return WrapError(err)
	}
