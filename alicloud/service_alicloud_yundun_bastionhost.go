...
func (s *YundunBastionhostService) DescribeBastionhostHostUserPublicKey(id string) (object map[string]interface{}, err error) {
	var response map[string]interface{}
	conn, err := s.client.NewBastionhostClient()
	if err != nil {
		return nil, WrapError(err)
	...
	addDebug(action, response, request)
	if err != nil {
		if IsExpectedErrors(err, []string{"Commodity.BizError.InvalidStatus", "OBJECT_NOT_FOUND", "ListUserPublicKeysNotFound"}) {
			return object, WrapErrorf(Error(GetNotFoundMessage("Bastionhost:ListUserPublicKeys", id)), NotFoundMsg, ProviderERROR, fmt.Sprint(response["RequestId"]))
		}
		return object, WrapErrorf(err, DefaultErrorMsg, id, action, AlibabaCloudSdkGoERROR)
	}
	keys, err := jsonpath.Get("$.PublicKeys", response)
	if err != nil {
		return object, WrapErrorf(err, FailedGetAttributeMsg, id, "$.PublicKeys", response)
	}

	publicKeyId := parts[2]
	for _, v := range keys.([]interface{}) {
		if v.(map[string]interface{})["PublicKeyId"] == publicKeyId {
			object = v.(map[string]interface{})
		}
	}
